...
class ClusterState(object):
    ...
    def keepalive(self):
        ...
        for nodeId, health in health_tmp.iteritems():
            ...
            ## remove node
            if (most - health) > 5 and sum(mask):#self.nodes.index({'session':node}):
    ...
    def addContainer(self, containerId, config):
        if config.has_key("CYGNET_INTERNAL_IP") and config.has_key("CYGNET_INTERNAL_SUBNET"):
    ...
    def stopContainer(self, containerId):
        if not containerId or type(containerId) not in [str,unicode]:
        ...
        if len(self.nodes) <= len(nodes) and len(self.nodes) == 1:
            for node in nodes:
                if node not in self.nodes:
                    ...
                    if not self.health.has_key(node['id']) and self.health:
    ...
    def syncRequest(self, origin):
        if not self.health.has_key(origin['id']):
